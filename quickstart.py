...
def daily_balance_check(date):
    ...
    print(date, last_date, last_balance)
    if last_date != date:
        batch_array = [{
            'range': f'A{last_date_row+1}',
            'values': [[date]],
        }, {
            'range': f'F{last_date_row+1}',
            'values': [['Daily Initial Balance']],
        }, {
            'range': f'L{last_date_row+1}',
            'values': [[last_balance]],
        }]
        expense_database.batch_update(batch_array)
